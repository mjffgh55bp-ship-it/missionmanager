...
export default function Workers() {
  ...
  const getWorkerTotalHours = (workerId) => {
    return assignments
      .filter(a => 
        a.chef_id === workerId || 
        a.sous_chef_id === workerId || 
        a.additional_chef_id === workerId
      )
  ...
  const handleSubmit = async () => {
    if (editingWorker) {
      ...
      await base44.entities.Worker.create(formData);
    }
    setShowDialog(false);
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      <div className={`w-12 h-12 rounded-full flex items-center justify-center ${
                        worker.role === 'chef' ? 'bg-blue-900' : 'bg-amber-500'
                      }`}>
                      ...
                      <div>
                        ...
                        <div className="flex gap-2 mt-1 flex-wrap">
                          ...
                          <Badge className={seniorityInfo.color}>
                            {seniorityInfo.label}
                          </Badge>
                          {worker.is_guide && (
                  ...
                  <div className="space-y-4">
                      ...
                      {progression.nextLevel ? (
                        <>
                          ...
                          <div className="flex items-center gap-2 text-xs text-gray-600">
                            ...
                            <span>{progression.hoursRemaining}h until next rank</span>
                          </div>
                        </>
                      ) : (
                    ...
                    <div className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-gray-900">Guide Status</span>
                      </div>
                      <Switch
                        checked={worker.is_guide}
                        onCheckedChange={() => toggleGuide(worker)}
                      />
                    </div>
                    ...
                    {worker.hire_date && (
                      <p className="text-sm text-gray-600">
                        📅 Hired: {format(new Date(worker.hire_date), "MMM d, yyyy")}
                      </p>
                    )}
                  </div>
        ...
        <Dialog open={showDialog} onOpenChange={setShowDialog}>
          <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                <Label htmlFor="is_guide" className="cursor-pointer flex items-center gap-2">
                  <Award className="w-4 h-4 text-yellow-600" />
                  <span>Qualified Guide</span>
                </Label>
                <Switch
                  id="is_guide"
                  checked={formData.is_guide}
                  onCheckedChange={(checked) => setFormData({ ...formData, is_guide: checked })}
                />
              </div>
              <div>
