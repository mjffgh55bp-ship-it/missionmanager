...
import { PinnedLayout, ClassicLayout } from "../components/matrix/MatrixLayouts";
...
export default function Matrix() {
  ...
  const [unavailDragging, setUnavailDragging] = useState(null); // {unavail, action, startAbsMinute, originalStart, originalEnd}
  ...
  const handleUnavailMouseDown = (e, unavail, action) => {
    e.stopPropagation();
    e.preventDefault();
    const slot = getSlotFromPointer(e);
    let startAbsMinute;
    ...
    setUnavailDragging({ unavail, action, startAbsMinute, originalStart: unavail.start_time, originalEnd: unavail.end_time, dayIndex: viewMode === 'weekly' ? getDayIndexFromDate(unavail.date) : 0 });
  };
  ...
  const handleMouseUp = async () => {
    // Unavailability resize save
    if (unavailDragging) {
      const { unavail, previewStart, previewEnd } = unavailDragging;
      setUnavailDragging(null);
      if (previewStart && previewEnd && previewStart !== unavail.start_time || previewEnd !== unavail.end_time) {
        const ns = previewStart || unavail.start_time;
        const ne = previewEnd || unavail.end_time;
        if (ns !== unavail.start_time || ne !== unavail.end_time) {
          setUnavailabilities(prev => prev.map(u => u.id === unavail.id ? { ...u, start_time: ns, end_time: ne } : u));
          try {
            if (!String(unavail.id).startsWith('temp_')) {
              await base44.entities.Unavailability.update(unavail.id, { start_time: ns, end_time: ne });
            }
          } catch (e) {
            console.error('update unavailability failed:', e);
            setUnavailabilities(prev => prev.map(u => u.id === unavail.id ? unavail : u));
          }
        }
      }
    }
  ...
  const AssignmentBar = ({ assignment }) => {
    ...
    // Resolve qualification name from worker_column_name
    const qualName = (() => {
      const colName = assignment.worker_column_name;
  ...
  const UnavailabilityBar = ({ unavail }) => {
    ...
    const isBeingDragged = unavailDragging?.unavail?.id === unavail.id;
    const displayStart = isBeingDragged && unavailDragging.previewStart ? unavailDragging.previewStart : unavail.start_time;
    const displayEnd = isBeingDragged && unavailDragging.previewEnd ? unavailDragging.previewEnd : unavail.end_time;
    const startPx = timeToPixels(displayStart, dayIndex, viewMode, ppm);
    const endPx = endTimeToPixels(displayStart, displayEnd, viewMode, ppm, dayIndex);
    const widthPx = Math.max(endPx - startPx, 0);
    const rightPx = startPx;
    if (startPx < 0 || startPx > timelineWidth) return null;
    const colorClass = unavail.reason === 'overseas' ? 'bg-red-200 border-red-500' : 'bg-gray-300 border-gray-500';
    return (
      <TooltipProvider><Tooltip><TooltipTrigger asChild>
        <div
          data-matrix-existing-bar="true"
          onClick={(e) => { if (isBeingDragged) return; e.stopPropagation(); openUnavailInUnifiedDialog(unavail); }}
          className={`absolute h-full rounded-sm flex items-center justify-center border-r-2 z-15 ${canManage ? 'cursor-pointer' : ''} ${colorClass} ${isBeingDragged ? 'opacity-80' : ''}`}
          style={{ right: `${rightPx}px`, width: `${widthPx}px` }}
        >
          <Ban className="w-3 h-3 text-gray-600" />
          {canManage && (
            <>
              <div data-matrix-existing-bar="true" className="absolute right-0 top-0 h-full cursor-ew-resize z-30" style={{ width: '12px', marginRight: '-4px' }} onMouseDown={(e) => { e.stopPropagation(); handleUnavailMouseDown(e, unavail, 'resize-start'); }} />
              <div data-matrix-existing-bar="true" className="absolute left-0 top-0 h-full cursor-ew-resize z-30" style={{ width: '12px', marginLeft: '-4px' }} onMouseDown={(e) => { e.stopPropagation(); handleUnavailMouseDown(e, unavail, 'resize-end'); }} />
            </>
          )}
        </div>
      </TooltipTrigger><TooltipContent className="bg-gray-800 text-white border-none">
        <p className="font-bold capitalize">{unavail.reason}</p><p>{displayStart} - {displayEnd}</p>
      </TooltipContent></Tooltip></TooltipProvider>
    );
  };
  ...
  const renderPinnedLayout = () => (
    <PinnedLayout {...sharedLayoutProps} workerPanelRef={workerPanelRef} timelineHeaderRef={timelineHeaderRef} timelineScrollRef={timelineScrollRef} />
  );

  const renderClassicLayout = () => (
    <ClassicLayout {...sharedLayoutProps} scrollContainerRef={scrollContainerRef} />
  );
